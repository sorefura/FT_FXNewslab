...
Return exactly `VERDICT: APPROVE` or `VERDICT: REQUEST_CHANGES` as the first non-empty line.
As the second non-empty line, copy `REVIEW_NONCE: <nonce>` exactly from the bundle.
For REQUEST_CHANGES, provide blocking P0/P1/P2 findings with exact evidence and required outcomes.
Approve only when the bundle proves the entire milestone complete; absence of an obvious defect is
not proof.
